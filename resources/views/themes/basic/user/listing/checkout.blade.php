									...
									<div class="transfer-details-section">
										<ul class="transfer-list">
											...
											<li class="item">
												...
												<h6>
													<span class="total_order_amount amount">{{$amount}}</span>
										...
										<button type="button" class="btn btn-custom w-100" id="confirmButton" data-bs-toggle="modal" data-bs-target="#staticBackdrop" data-listingTitle="{{__($listing_title)}}" data-listingId="{{$listing_id}}" data-order_id="{{$order_id}}">@lang('confirm and continue')</button>
									</div>
								</div>
							</div>
						</div>
	...
	<div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form action="{{ route('fund.initialize.listing',["slug"=>$listing_title,"id"=>$listing_id]) }}" method="post">
					@csrf
...
@push('script')
	<script>
		...
		$("#confirmButton").click(function() {
			...
			let listingTitle = $(this).attr('data-listingTitle');
			let listingId = $(this).attr('data-listingId');
			let orderId = $(this).attr('data-order_id');

			$.ajax({
				url:"{{route('listing.gateway')}}",
				type:"GET",
				data:{
					methodId:methodId,
